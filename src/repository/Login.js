...
exports.login = async function login(user) {
    ...
    try {
        ...
        if(result.length == 0) response = {code: 401,  data: {}, message: "Identifiant ou mot de passe invalide"};
        else {
        ...
        response = {code: 501,  data: {}, message: e.message};
        console.error(e);
    } finally {
        await client.close();
    }
...
exports.verifyToken = async function verifyToken(token) {
    ...
    try {
        ...
        if(result.length == 0) response = {code: 401,  data: {}, message: "Veuillez vous connecter"};
