...
exports.login = async function login(user) {
    ...
    try {
        ...
        if(result.length == 0) response = {code: 401,  data: {"data":0}, message: "Identifiant ou mot de passe invalide"};
        else {
        ...
        response = {code: 501,  data: {"data":0}, message: e.message};
        console.error(e);
    } finally {
        await client.close();
    }
...
exports.verifyToken = async function verifyToken(token) {
    ...
    try {
        ...
        if(result.length == 0) response = {code: 401,  data: {"data":0}, message: "Veuillez vous connecter"};
